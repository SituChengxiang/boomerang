...
def load_data():
    ...
    if not csv_files:
        print("No data/*opt.csv files found.")
        return {}
    ...
    for f in csv_files:
        ...
        # Using a weighted average of first 5 steps
        dt_start = df.t.iloc[1] - df.t.iloc[0] # Assume constant roughly
        if dt_start == 0: continue
        ...
        for i in range(5):
            if i+1 >= len(df): break
            dt = df.t.iloc[i+1] - df.t.iloc[i]
            if dt == 0: continue
            wx = (df.x.iloc[i+1] - df.x.iloc[i]) / dt
            wy = (df.y.iloc[i+1] - df.y.iloc[i]) / dt
            wz = (df.z.iloc[i+1] - df.z.iloc[i]) / dt
            vx0 += wx * weights[i]
            vy0 += wy * weights[i]
            vz0 += wz * weights[i]
        ...
        tracks[key] = {
            ...
            'omega': float(TRACK_META[key]),
        }

        # Precompute measured velocity from the smoothed positions.
        # This lets the fitter constrain heading/turning and end-segment speeds.
        t = tracks[key]['t']
        pos = tracks[key]['pos']
        if len(t) >= 5:
            vx_m = np.gradient(pos[:, 0], t)
            vy_m = np.gradient(pos[:, 1], t)
            vz_m = np.gradient(pos[:, 2], t)
            tracks[key]['vel_meas'] = np.stack([vx_m, vy_m, vz_m], axis=1)

            ax_m = np.gradient(vx_m, t)
            ay_m = np.gradient(vy_m, t)
            az_m = np.gradient(vz_m, t)
            tracks[key]['acc_meas'] = np.stack([ax_m, ay_m, az_m], axis=1)
        else:
            tracks[key]['vel_meas'] = np.zeros_like(pos)
            tracks[key]['acc_meas'] = np.zeros_like(pos)
    return tracks
...
def boomerang_rhs(t, state, CL, CD, phi_base, k_bank, v0_scalar, omega_scale):
    ...
    phi = phi_base + (k_bank * omega_scale) * speed_loss + (PHI_TIME_GAIN * omega_scale) * float(t)
    ...
    ax_lift, ay_lift, az_lift = float(lift_vec[0]), float(lift_vec[1]), float(lift_vec[2])
    ...
    cd_reduction = 0.25
    CD_eff = CD * (1.0 - cd_reduction * dive_ratio) * (1.0 + BANK_DRAG_GAIN * (np.sin(phi) ** 2))
...
def integrate_rk4(t_eval: np.ndarray, state0: np.ndarray, args, substeps: int) -> np.ndarray:
    n = len(t_eval)
...
if _HAVE_NUMBA:
    ...
    def boomerang_rhs_nb(t, state, CL, CD, phi_base, k_bank, v0_scalar, omega_scale):
        ...
        lift0_x, lift0_y, lift0_z = _cross_nb(lat_x, lat_y, lat_z, v_hat_x, v_hat_y, v_hat_z)
    ...
    @njit(cache=True, fastmath=True)
    def integrate_rk4_nb(t_eval, state0, CL, CD, phi_base, k_bank, v0_scalar, omega_scale, substeps):
        n = t_eval.shape[0]
        states = np.empty((n, 6), dtype=np.float64)
        states[0, :] = state0
        y = state0.copy()
        ...
        for i in range(n - 1):
            ...
            tt = t0
            for _ in range(substeps):
                k1 = boomerang_rhs_nb(tt, y, CL, CD, phi_base, k_bank, v0_scalar, omega_scale)
                k2 = boomerang_rhs_nb(tt + 0.5 * h, y + 0.5 * h * k1, CL, CD, phi_base, k_bank, v0_scalar, omega_scale)
                k3 = boomerang_rhs_nb(tt + 0.5 * h, y + 0.5 * h * k2, CL, CD, phi_base, k_bank, v0_scalar, omega_scale)
                k4 = boomerang_rhs_nb(tt + h, y + h * k3, CL, CD, phi_base, k_bank, v0_scalar, omega_scale)
...
def simulate_track(params, track_data, return_full=False):
    ...
    try:
        if engine == "ivp":
            ...
            sol = solve_ivp(
                lambda tt, yy: boomerang_rhs(tt, yy, CL, CD, phi_base, k_bank, v0_xy_scalar, omega_scale),
...
def loss_function(params, tracks):
    ...
    for key, data in tracks.items():
        ...
        real_acc = data.get('acc_meas', np.zeros_like(real_pos))
...
def fit_multistart(tracks, bounds, x0, starts, seed):
    ...
    for i, guess in enumerate(initial_points, start=1):
        res = minimize(loss_function, guess, args=(tracks,), method='L-BFGS-B', bounds=bounds)
        if res.success and res.fun < best_fun:
            best_fun = float(res.fun)
            best_x = res.x
            best_res = res
        print(f"[start {i}/{len(initial_points)}] loss={res.fun:.4f} success={res.success}")

    return best_res, best_x


def _optimize_one_start(guess, tracks, bounds, start_timeout_s, maxiter, engine, substeps):
...
def fit_multistart_parallel(tracks, bounds, x0, starts, seed, workers, start_timeout_s, maxiter):
    rng = np.random.default_rng(seed)
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as ex:
        ...
        for idx, guess in enumerate(initial_points, start=1):
            fut = ex.submit(_optimize_one_start, guess, tracks, bounds, start_timeout_s, maxiter, SIM_ENGINE, RK_SUBSTEPS)
            futures[fut] = idx
        ...
        for fut in as_completed(futures):
            ...
            if np.isfinite(out['fun']):
                print(f"[start {idx}/{len(initial_points)}] loss={out['fun']:.4f} success={out['success']}{msg}")
            else:
                print(f"[start {idx}/{len(initial_points)}] loss=inf success={out['success']}{msg}")
...
def main():
    ...
    parser.add_argument("--no-plot", action="store_true", help="Fit only, do not show matplotlib windows")
    parser.add_argument("--starts", type=int, default=max(12, (os.cpu_count() or 12)), help="Number of multistart initializations")
    parser.add_argument("--seed", type=int, default=0, help="RNG seed for multistart")
    parser.add_argument("--workers", type=int, default=0, help="Parallel worker processes for multistart (0=auto)")
    parser.add_argument("--start-timeout", type=int, default=90, help="Per-start timeout seconds (0=disable)")
    parser.add_argument("--maxiter", type=int, default=120, help="Max L-BFGS-B iterations per start")
    parser.add_argument("--track", type=str, default="", help="Only report/plot a single track key (e.g. track1)")
    parser.add_argument("--report", action="store_true", help="Print per-track error summary and tail metrics")
    parser.add_argument("--save-plots", type=str, default="", help="Directory to save plots as PNG instead of only showing")
    parser.add_argument("--export-csv", type=str, default="", help="Export per-time-step diagnostics CSV (requires --track)")
    parser.add_argument("--turn", choices=["right", "left", "free"], default="right", help="Constrain turning direction via phi_base bounds")
    parser.add_argument(
        "--engine",
    ...
    if SIM_ENGINE == "rk4-numba" and not _HAVE_NUMBA:
        print("[warn] --engine=rk4-numba requested but numba not available; falling back to rk4")
    ...
    def compute_track_metrics(track_key, best_params):
        ...
        vy_tail_rmse = float(np.sqrt(np.mean((np.abs(sim_vel[tail, 1]) - np.abs(real_vel[tail, 1])) ** 2)))
    ...
    if args.export_csv:
        if not args.track:
            ...
            phi_hist = np.clip(phi_base + (k_bank * omega_scale) * (v0_scalar - v_xy_hist) + (PHI_TIME_GAIN * omega_scale) * t, -1.5, 1.5)

            out_df = pd.DataFrame({
                "t": t,
                "x_real": real_pos[:, 0], "y_real": real_pos[:, 1], "z_real": real_pos[:, 2],
                "x_sim": sim_pos[:, 0], "y_sim": sim_pos[:, 1], "z_sim": sim_pos[:, 2],
                "vx_real": real_vel[:, 0], "vy_real": real_vel[:, 1], "vz_real": real_vel[:, 2],
                "vx_sim": sim_vel[:, 0], "vy_sim": sim_vel[:, 1], "vz_sim": sim_vel[:, 2],
    ...
    for idx, key in enumerate(valid_keys, start=1):
        ...
        ax.plot(real_pos[:, 0], real_pos[:, 1], real_pos[:, 2], 'k--', label='Tracker Data', alpha=0.5)
        ax.plot(sim_pos[:, 0], sim_pos[:, 1], sim_pos[:, 2], 'r-', label='Fit Model', linewidth=2)
        ...
        v0_scalar = float(np.sqrt(tracks[key]['v0'][0]**2 + tracks[key]['v0'][1]**2))
        omega_scale = float(tracks[key]['omega'] / OMEGA_REF)
        phi_hist = np.degrees(np.clip(phi_base + (k_bank * omega_scale) * (v0_scalar - v_xy_hist), -1.5, 1.5))
        
        ax3 = ax.plot([], [], [], alpha=0.0)  # keep layout stable; phi plotted on same right axis as |vy|
