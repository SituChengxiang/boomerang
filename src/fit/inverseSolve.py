...
a = 0.15      # m (arm length)
d = 0.028     # m (arm width)
S = 2 * a * d # m^2, planform area. 2 wings * length * width
rho = 1.225   # kg/m^3
g = 9.793     # m/s^2

# Rotation parameters (New Physics Model)
SIGMA_ROTATION = 0.4    # Rotary lift contribution factor
OMEGA0 = 85.0           # rad/s, initial rotation
OMEGA_DECAY = 1.5       # rad/s^2, linear decay rate
...
def solve_coefficients(file_path):
    ...
    # g points down in Z? User said "z up is positive". So gravity is [0, 0, -g]
    F_g = np.zeros_like(F_net)
    F_g[:, 2] = -g * m  # Fix: Gravity force is m*g
...
def plot_and_save(results, output_prefix, output_dir):
    ...
    ld_ratio = np.divide(cl_vals, cd_vals, out=np.full_like(cl_vals, np.nan, dtype=float), where=valid_ld)
...
if __name__ == "__main__":
    ...
    for file_path in files:
        ...
        stats = res[mask][["Cl", "Cd", "Cd_signed"]].describe()
